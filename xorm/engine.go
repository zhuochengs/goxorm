...
// ShowSelectSQL show SQL statement or not on logger if log level is great than INFO
func (engine *Engine) ShowSelectSQL(show ...bool) {
	engine.logger.ShowSelectSQL(show...)
	if len(show) == 0 {
		engine.showSelectSQL = true
	} else {
		engine.showSelectSQL = show[0]
	}
}
