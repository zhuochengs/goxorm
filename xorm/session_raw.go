...
func (session *Session) queryRows(sqlStr string, args ...interface{}) (*core.Rows, error) {
	...
	if session.engine.showSQL {
		// if it's not containt SELECT prefix, OR contanit and showSelectSQL, log
		testShows := strings.TrimLeft(sqlStr, " ")
		if !strings.HasPrefix(testShows, "SELECT ") ||
			(strings.HasPrefix(testShows, "SELECT ") && session.engine.showSelectSQL) {
